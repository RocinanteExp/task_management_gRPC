...
const TodoItem = (props) => {
    ...
    let { mode, task, deleteTask, updateTask, selectTask, usersList, unassignTask } = props;
    ...
    return (
        <ListGroup.Item id={task.id}>
            <div className="d-flex w-100 justify-content-between">
                ...
                {!task.privateTask && mode === "public" && <small>{task.user}</small>}
                {!task.privateTask && (
                    <svg
                ...
                {task.deadline && (
                    <small className={task.deadline.isBefore(moment()) ? "date bg-danger text-white" : "date"}>
                        {task.deadline.format("dddd, MMMM Do YYYY, h:mm:ss a")}{" "}
                    </small>
                )}
